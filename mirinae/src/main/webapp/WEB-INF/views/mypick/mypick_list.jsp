...
<c:if test="${pick_list.size() != 0  }">
		<c:forEach var="list" items="${pick_list }">
			...
			<tr><td colspan="2" class="underline">
				<c:if test="${list.cate_code == 100 }">가전</c:if>
				<c:if test="${list.cate_code == 200 }">반려동물</c:if>
				<c:if test="${list.cate_code == 300 }">뷰티</c:if>
				<c:if test="${list.cate_code == 400 }">푸드</c:if>
				<c:if test="${list.cate_code == 500 }">패션</c:if>
				<c:if test="${list.cate_code == 600 }">기타</c:if>
			</td></tr>
