...
<c:if test="${not empty project }">
		...
		<c:if test="${pro.p_del != 'y' }">
			...
			<a href="/mirinae/views/project/p_view.kim?p_no=${pro.p_no }">
...
<c:if test="${startPage > PAGE_PER_BLOCK }">
	<button class="btn btn-secondary" onclick="location.href='../main/homeSearch.kwon?pageNum=${startPage-1 }&search=${search }'">이전</button>
</c:if>
<c:forEach var="i" begin="${ startPage }" end="${endPage }">
	<c:if test="${currentPage == i }">
		<button class="btn btn-secondary" onclick="location.href='../main/homeSearch.kwon?pageNum=${i}&search=${search }'" disabled="disabled">${i }</button>
	</c:if>
	<c:if test="${currentPage != i }">
		<button class="btn btn-secondary" onclick="location.href='../main/homeSearch.kwon?pageNum=${i}&search=${search }'">${i }</button>	
	</c:if>
</c:forEach>
<c:if test="${endPage < totalPage }">
	<button class="btn btn-secondary" onclick="location.href='../main/homeSearch.kwon?pageNum=${endPage+1 }&search=${search }'">다음</button>
</c:if>
</div>
</body>
</html>