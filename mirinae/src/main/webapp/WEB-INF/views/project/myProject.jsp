...
<c:if test="${empty myProject }">
		<h1 align="center">등록한 프로젝트가 없습니다.</h1>
</c:if>
<c:if test="${not empty myProject }">
		<c:forEach var="pro" items="${myProject }">
...
<c:if test="${startPage > PAGE_PER_BLOCK }">
	<button class="btn btn-secondary" onclick="location.href='myProject.la?pageNum=${startPage-1 }'">이전</button>
</c:if>
<c:forEach var="i" begin="${ startPage }" end="${endPage }">
	<c:if test="${currentPage == i }">
		<button class="btn btn-secondary" onclick="location.href='myProject.la?pageNum=${i}'" disabled="disabled">${i }</button>
	</c:if>
	<c:if test="${currentPage != i }">
		<button class="btn btn-secondary" onclick="location.href='myProject.la?pageNum=${i}'">${i }</button>	
	</c:if>
</c:forEach>
<c:if test="${endPage < totalPage }">
	<button class="btn btn-secondary" onclick="location.href='myProject.la?pageNum=${endPage+1 }'">다음</button>
</c:if>
</div>
</body>
</html>