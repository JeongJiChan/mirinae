...
<style type="text/css">
	table { border: 1px solid black; margin: 30px;}
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#cate").val('${cate_code}').prop("selected", true);
...
<c:if test="${startPage > PAGE_PER_BLOCK }">
	<button onclick="location.href='p_cateView.kim?pageNum=${startPage-1 }&cate_code=${cate_code }'">이전</button>
</c:if>
<c:forEach var="i" begin="${ startPage }" end="${endPage }">
	<c:if test="${currentPage == i }">
		<button onclick="location.href='p_cateView.kim?pageNum=${i}&cate_code=${cate_code }'" disabled="disabled">${i }</button>
	</c:if>
	<c:if test="${currentPage != i }">
		<button onclick="location.href='p_cateView.kim?pageNum=${i}&cate_code=${cate_code }'">${i }</button>	
	</c:if>
</c:forEach>
<c:if test="${endPage < totalPage }">
	<button onclick="location.href='p_cateView.kim?pageNum=${endPage+1 }&cate_code=${cate_code }'">다음</button>
</c:if>
</div>
</body>
</html>